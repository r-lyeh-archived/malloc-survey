...
#ifdef _WIN32
#define ATTRIBUTE(x)
#define BA_OS_HEADER <windows.h>
#define INLINE __inline
#else
#define BA_OS_HEADER <unistd.h>
#endif
...
# else
#  include <stdio.h>
#  include BA_OS_HEADER
ATTRIBUTE((noreturn))
static INLINE void __error(int line, char * file, char * msg) {
    fprintf(stderr, "%s:%d\t%s\n", file, line, msg);
    _exit(1);
}
#  define ba_error(...)	do { snprintf(errbuf, 127, __VA_ARGS__); __error(__LINE__, __FILE__, errbuf); } while(0)
...
#ifdef BA_DEBUG
# define BA_ERROR(a, ...)	do {				\
    fprintf(stderr, "ERROR at %s:%d\n", __FILE__, __LINE__);	\
    if (a) {							\
	ba_print_htable(a);					\
	ba_show_pages(a);					\
    }								\
    ba_error(__VA_ARGS__);						\
} while(0)
#else
# define BA_ERROR(a, ...)	do { ba_error(__VA_ARGS__); } while(0)
#endif
